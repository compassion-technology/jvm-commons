...
public class SWTUtils {
    ...
    public static java.awt.Font toAwtFont(Device device, FontData fontData,
        ...
        if (ensureSameSize) {
            ...
            if (dummyPanel.getFontMetrics(tmpAwtFont).stringWidth(Az)
                    > tmpGC.textExtent(Az).x) {
                while (dummyPanel.getFontMetrics(tmpAwtFont).stringWidth(Az)
                        > tmpGC.textExtent(Az).x) {
                    height--;
            ...
            else if (dummyPanel.getFontMetrics(tmpAwtFont).stringWidth(Az)
                    < tmpGC.textExtent(Az).x) {
                while (dummyPanel.getFontMetrics(tmpAwtFont).stringWidth(Az)
                        < tmpGC.textExtent(Az).x) {
                    height++;
     ...
     */
    public static MouseEvent toAwtMouseEvent(Component src, org.eclipse.swt.events.MouseEvent event) {
        ...
        return new MouseEvent(src, event.hashCode(), event.time & 0xFFFFFFFFL,
        		modifiers, event.x, event.y, event.count, false, button);
    }
