...
public class ConfigFactory<C extends Config> extends YamlConfigurationFactory<C> {
	...
	@Setter
	private ObjectMapper loggingMapper = new ObjectMapper();
	@Setter
	...
	public static class ConfigFactoryFactory<T extends Config> extends DefaultConfigurationFactoryFactory<T> {
		@Override
		public ConfigurationFactory<T> create(Class<T> klass, Validator validator, ObjectMapper mapper, String prefix) {
			return new ConfigFactory<>(klass, validator, configureObjectMapper(mapper), prefix);
		}
	}
}
